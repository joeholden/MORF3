...
vessel_distances_data_path = "C:/Users/Acer/PycharmProjects/MORF3/DATA/" \
                             "All Vessel Distance Excel Sheets/Consolidated Vessel Distances.xlsx"
...
vessel_distances_df = pd.read_excel(vessel_distances_data_path)
vessel_distances_df.drop(['Image'], axis=1)
...
all_dataframes = [retistruct_df, full_outline_df, convex_hull_df, vessel_distances_df]
merged_data = ft.reduce(lambda left, right: pd.merge(left, right, on="Cell Identifier"), all_dataframes)

merged_data.to_excel('Merged Data_27.xlsx', index=False)